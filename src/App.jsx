...
// ymdLocal: convierte un Date a “YYYY-MM-DD” usando hora LOCAL (no UTC).
// Importante para evitar bugs de offset: new Date(2026, 4, 1).toISOString()
// devuelve “2026-04-30T22:00:00Z” en horario CEST y eso rompe los rangos
// de filtrado (start de mayo aparecería como 30 abril).
const ymdLocal = (d) => {
if (!d) return “”;
...
watches: [],
opportunities: [],
offers: [],  // Bot scanner offers (from Excel via Apps Script)
price_comps: [],
expenses: [],
...
…state,
customer_notes: state.customer_notes.filter((*, i) => i !== action.index),
};
...
case “DELETE_EXPENSE”: return { …state, expenses: state.expenses.filter((*, i) => i !== action.index) };
...
if (period === "today") {
  ...
  start = ymdLocal(firstOfMonth);
  end = (monthOffset === 0) ? todayStr : ymdLocal(lastOfMonth);
  ...
  prevStart = ymdLocal(firstOfPrev);
  prevEnd = ymdLocal(cutoffPrev);
  ...
  start = ymdLocal(firstOfQ); end = todayStr;
  label = `${cq + 1}T ${y}`;
  ...
  prevStart = ymdLocal(firstOfPrevQ);
  prevEnd = ymdLocal(cutoffPrevQ);
  prevLabel = `${cq}T ${firstOfPrevQ.getFullYear()}`;
  daysElapsed = elapsed;
} else if (period === "year") {
  const firstOfY = new Date(y, 0, 1);
  start = ymdLocal(firstOfY); end = todayStr;
  label = `${y}`;
...
const last30Sales = state.watches.filter(w =>
  w.status === "sold" && w.sale_date >= ymdLocal(new Date(Date.now() - 30 * 86400000))
);
...
```
  ...
  {/* Photos */}
  <PhotoGallery watchId={watch.id} watch={watch} />
...
// PHOTO GALLERY
// ═══════════════════════════════════════════════════════════
const PhotoGallery = ({ watchId, watch }) => {
...
const accountingPhoto = watch && watch.accounting_photo_url;
...
<span className=“text-xs tracking-widest uppercase font-bold” style={{ color: C.mute }}>Foto principal</span>
<span className=“text-[10px] ml-auto” style={{ color: C.dim }}>desde contabilidad</span>
</div>
<button onClick={() => setLightbox(-1)} className=“block w-full rounded-xl overflow-hidden” style={{ border: `1px solid ${C.gold}44` }}>
<img src={accountingPhoto} alt="" className="w-full aspect-square object-cover" />
</button>
</div>
)}
...
```
  ...
  {lightbox !== null && (lightbox === -1 ? accountingPhoto : photos[lightbox]) && (
    <div onClick={() => setLightbox(null)} className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.92)" }}>
      ...
      <img src={lightbox === -1 ? accountingPhoto : (photos[lightbox].url || photos[lightbox].data)} alt="" className="max-w-full max-h-full rounded-xl" onClick={(e) => e.stopPropagation()} />
      {lightbox !== -1 && photos.length > 1 && (
        <>
...
// VIEW: ADD/EDIT WATCH
// ═══════════════════════════════════════════════════════════
const WatchForm = ({ state, dispatch, editId, prefill, setView, goBack }) => {
const editing = editId ? state.watches.find(w => w.id === editId) : null;
const [w, setW] = useState(editing || {
brand: prefill?.brand || “”, model: prefill?.model || “”, mechanism: “Automático”, year: “”, size: “”,
condition: “Buena”, purchase_price: prefill?.purchase_price || 0, purchase_date: today(),
purchase_source: prefill?.purchase_source || “”, notes: prefill?.notes || “”, target_channel: “Catawiki”, status: “stock”
});
...
// ═══════════════════════════════════════════════════════════════════════════
// OFFERS INBOX — Vista de ofertas del bot scanner Python
// ═══════════════════════════════════════════════════════════════════════════
// Las ofertas vienen de la pestaña OFERTAS del CRM Database, alimentada por el
// bot Python (eBay/Vinted/CashConverters/Wallapop scanners).
// El usuario decide: ignorar / pujar / comprar.
const OffersInboxView = ({ state, dispatch, setView }) => {
const offers = state.offers || [];

const [tab, setTab] = useState(“pendiente”); // pendiente | pujar | comprar | ignorar
const [sourceFilter, setSourceFilter] = useState(“all”); // all | eBay | Vinted | CashConverters | Wallapop
...
const result = { pendiente: 0, pujar: 0, comprar: 0, ignorar: 0 };
offers.forEach(o => {
const d = (o.decision_atelier || “pendiente”).toLowerCase();
...
const filtered = useMemo(() => {
return offers
.filter(o => (o.decision_atelier || “pendiente”).toLowerCase() === tab)
.filter(o => sourceFilter === “all” ? true : (o.source || “”) === sourceFilter)
.filter(o => {
const roiPct = (o.roi || 0) * 100;
return roiPct >= minRoi;
})
.filter(o => (o.score || 0) >= minScore)
.filter(o => {
if (!hideBlocked) return true;
const v = (o.vision_verdict || “”).toLowerCase();
return v !== “blocked”;
})
.sort((a, b) => {
// Sort by net_profit DESC then by score DESC
const np = (b.net_profit || 0) - (a.net_profit || 0);
if (np !== 0) return np;
return (b.score || 0) - (a.score || 0);
});
}, [offers, tab, sourceFilter, minRoi, minScore, hideBlocked]);

// KPIs
const kpis = useMemo(() => {
const totalOffers = offers.length;
const decided = offers.filter(o => (o.decision_atelier || “pendiente”).toLowerCase() !== “pendiente”);
const compradas = offers.filter(o => (o.decision_atelier || “”).toLowerCase() === “comprar”);
const conversionRate = totalOffers > 0 ? Math.round((compradas.length / totalOffers) * 100) : 0;
const avgRoiCompradas = compradas.length > 0
? compradas.reduce((s, o) => s + (o.roi || 0), 0) / compradas.length * 100
: 0;
const pendientes = totalOffers - decided.length;
return { totalOffers, decided: decided.length, compradas: compradas.length, conversionRate, avgRoiCompradas, pendientes };
}, [offers]);
...
const updated = state.offers.map(o => o.id === offer.id ? { …o, decision_atelier: decision, decision_date: new Date().toISOString() } : o);
dispatch({ type: “LOAD”, payload: { …state, offers: updated, loaded: true } });

```
  // Sync to backend
  await updateOfferDecisionGas({ id: offer.id, decision: decision });
} catch (err) {
...
<div className="flex items-baseline justify-between pt-2">
<div>
<h1 className=“font-serif text-3xl” style={{ color: C.cream, fontFamily: “‘Fraunces’, serif”, fontWeight: 300 }}>Ofertas</h1>
<div className=“text-xs mt-1” style={{ color: C.dim }}>
{kpis.pendientes} pendientes · {kpis.compradas} compradas · {kpis.decided - kpis.compradas} descartadas
</div>
</div>
<Btn onClick={() => setView({ name: “opportunities-manual” })} icon={Plus} variant=“secondary”>Manual</Btn>
</div>
...
```
  ...
  <div className="grid grid-cols-3 gap-2">
    <Card className="p-3">
      <div className="text-[10px] tracking-widest uppercase font-bold" style={{ color: C.mute }}>Total</div>
      <div className="font-serif text-2xl mt-1" style={{ color: C.cream, fontFamily: "'Fraunces', serif", fontWeight: 300 }}>{kpis.totalOffers}</div>
    </Card>
    <Card className="p-3">
      <div className="text-[10px] tracking-widest uppercase font-bold" style={{ color: C.mute }}>Conversión</div>
      <div className="font-serif text-2xl mt-1" style={{ color: C.gold, fontFamily: "'Fraunces', serif", fontWeight: 300 }}>{kpis.conversionRate}%</div>
    </Card>
    <Card className="p-3">
      <div className="text-[10px] tracking-widest uppercase font-bold" style={{ color: C.mute }}>ROI compras</div>
      <div className="font-serif text-2xl mt-1" style={{ color: C.jade, fontFamily: "'Fraunces', serif", fontWeight: 300 }}>{Math.round(kpis.avgRoiCompradas)}%</div>
    </Card>
  </div>
  ...
  <div className="flex gap-1.5 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
    {[
      ...
      { id: "pujar", label: "Pujadas", count: counts.pujar },
      { id: "comprar", label: "Compradas", count: counts.comprar },
      { id: "ignorar", label: "Descartadas", count: counts.ignorar },
    ].map(t => (
      <button key={t.id} onClick={() => setTab(t.id)}
        className="py-2 px-3.5 rounded-xl text-xs font-semibold tracking-wide transition-all whitespace-nowrap flex-shrink-0"
        style={{
  ...
  <button onClick={() => setShowFilters(!showFilters)}
    ...
    style={{ background: C.raised, border: `1px solid ${C.line}`, color: C.dim }}>
    <Filter size={12} />
    Filtros {(sourceFilter !== "all" || minRoi !== 30 || minScore !== 60 || !hideBlocked) ? "(activos)" : ""}
  </button>
  ...
  {showFilters && (
    <Card className="p-3 space-y-3">
      {/* Source filter */}
      <div>
        <div className="text-[10px] tracking-widest uppercase mb-2" style={{ color: C.mute }}>Fuente</div>
        <div className="flex gap-1.5 flex-wrap">
          {["all", ...sources].map(s => (
            <button key={s} onClick={() => setSourceFilter(s)}
              className="py-1.5 px-3 rounded-lg text-xs"
              style={{
                background: sourceFilter === s ? C.gold : "transparent",
                color: sourceFilter === s ? C.ink : C.dim,
                border: `1px solid ${sourceFilter === s ? C.gold : C.line}`,
              }}>
              {s === "all" ? "Todas" : s}
            </button>
          ))}
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <div className="text-[10px] tracking-widest uppercase" style={{ color: C.mute }}>ROI mínimo</div>
          <div className="text-xs" style={{ color: C.gold }}>{minRoi}%</div>
        </div>
        <input type="range" min="0" max="200" step="5" value={minRoi} onChange={e => setMinRoi(Number(e.target.value))}
          className="w-full" style={{ accentColor: C.gold }} />
      </div>
      {/* Score slider */}
      <div>
        <div className="flex items-center justify-between mb-1">
          <div className="text-[10px] tracking-widest uppercase" style={{ color: C.mute }}>Score mínimo</div>
          <div className="text-xs" style={{ color: C.gold }}>{minScore}</div>
        </div>
      ...
      <label className="flex items-center gap-2 text-xs" style={{ color: C.dim }}>
        <input type="checkbox" checked={hideBlocked} onChange={e => setHideBlocked(e.target.checked)} style={{ accentColor: C.gold }} />
        Ocultar Vision: blocked
      </label>
    </Card>
  )}
  ...
  {filtered.length === 0 && (
    <Card className="p-6 text-center">
      <div className="text-sm" style={{ color: C.dim }}>
        {offers.length === 0
          ? "No hay ofertas del bot todavía. Ejecuta el scanner desde GitHub Actions."
  ...
  {filtered.map(offer => (
    <OfferCard
      ...
      offer={offer}
      busy={busy === offer.id}
      onIgnore={() => decide(offer, "ignorar")}
      onBid={() => decide(offer, "pujar")}
      onBuy={() => handleBuy(offer)}
      onClick={() => setDetailOffer(offer)}
      showActions={tab === "pendiente"}
    />
  ))}

  {/* Detail modal */}
  {detailOffer && (
    <Modal title="Detalle oferta" onClose={() => setDetailOffer(null)}>
...
const OfferCard = ({ offer, busy, onIgnore, onBid, onBuy, onClick, showActions }) => {
const roiPct = Math.round((offer.roi || 0) * 100);
const visionColor = (offer.vision_verdict || “”).toLowerCase() === “ok” ? C.jade
: (offer.vision_verdict || “”).toLowerCase() === “uncertain” ? C.amber
: (offer.vision_verdict || “”).toLowerCase() === “blocked” ? C.ruby
: C.mute;

return (
<Card className="p-3 space-y-2.5">
<div className=“flex gap-3” onClick={onClick} style={{ cursor: “pointer” }}>
{/* Photo */}
<div className=“w-20 h-20 rounded-lg overflow-hidden flex-shrink-0” style={{ background: C.raised }}>
{offer.photo_url ? (
<img src={offer.photo_url} alt=”” className=“w-full h-full object-cover” onError={(e) => { e.target.style.display = “none”; }} />
) : (
<div className=“w-full h-full flex items-center justify-center” style={{ color: C.dim }}>
<Watch size={24} />
</div>
)}
</div>
...
```
    ...
    <div className="flex-1 min-w-0">
      <div className="flex items-start justify-between gap-2">
        <div className="font-serif text-base leading-tight truncate" style={{ color: C.cream, fontFamily: "'Fraunces', serif" }}>
          {offer.brand || "—"} {offer.model && `· ${offer.model}`}
        </div>
        <Chip color={C.gold}>{offer.source}</Chip>
      </div>
      {offer.title && (
      ...
      <div className="flex items-center gap-2 mt-1.5 text-xs flex-wrap">
        <span style={{ color: C.cream }}>€{Math.round(offer.price_eur || 0)}</span>
        <span style={{ color: C.mute }}>→</span>
        <span style={{ color: C.gold }}>~€{Math.round(offer.est_close || 0)}</span>
        <span className="font-semibold" style={{ color: roiPct > 80 ? C.jade : roiPct > 30 ? C.gold : C.dim }}>
          {roiPct}% ROI
        </span>
      </div>
      <div className="flex items-center gap-2 mt-1 text-[10px]" style={{ color: C.mute }}>
        <span>Score {offer.score || 0}</span>
        <span>·</span>
        <span style={{ color: visionColor }}>Vision {offer.vision_verdict || "?"}</span>
        {offer.location && <><span>·</span><span>{offer.location}</span></>}
      </div>
    </div>
  </div>
  ...
  {showActions && (
    <div className="flex gap-2 pt-1">
      <button onClick={onIgnore} disabled={busy}
        ...
        style={{ background: "transparent", border: `1px solid ${C.line}`, color: C.dim, opacity: busy ? 0.5 : 1 }}>
        Ignorar
      </button>
      <button onClick={onBid} disabled={busy}
        className="flex-1 py-2 rounded-lg text-xs font-semibold transition-all"
        style={{ background: `${C.amber}1a`, border: `1px solid ${C.amber}`, color: C.amber, opacity: busy ? 0.5 : 1 }}>
        Pujar
      </button>
      <button onClick={onBuy} disabled={busy}
        className="flex-1 py-2 rounded-lg text-xs font-semibold transition-all"
        style={{ background: C.gold, border: `1px solid ${C.gold}`, color: C.ink, opacity: busy ? 0.5 : 1 }}>
        Comprar
      </button>
    </div>
  )}
...
<div className=“space-y-3 text-sm” style={{ color: C.cream }}>
{offer.photo_url && (
<img src={offer.photo_url} alt=”” className=“w-full rounded-lg” onError={(e) => { e.target.style.display = “none”; }} />
)}
<div>
<div className=“text-[10px] tracking-widest uppercase” style={{ color: C.mute }}>Título original</div>
<div className=“text-xs mt-1” style={{ color: C.dim }}>{offer.title || “—”}</div>
</div>
<div>
<div className=“text-[10px] tracking-widest uppercase” style={{ color: C.mute }}>Identificado por bot</div>
<div className=“font-serif text-base mt-1” style={{ color: C.cream, fontFamily: “‘Fraunces’, serif” }}>
{offer.brand} · {offer.model} {offer.reference && `(ref ${offer.reference})`}
</div>
</div>
<div className=“grid grid-cols-2 gap-2 pt-2” style={{ borderTop: `1px solid ${C.line}` }}>
<Row label="Fuente" value={offer.source} />
<Row label=“Ubicación” value={offer.location || “—”} />
<Row label=“Precio compra” value={`€${Math.round(offer.price_eur || 0)}`} />
<Row label=“Cierre estimado” value={`€${Math.round(offer.est_close || 0)}`} />
<Row label=“Beneficio neto” value={`€${Math.round(offer.net_profit || 0)}`} valColor={C.jade} />
<Row label=“ROI” value={`${roiPct}%`} valColor={roiPct > 80 ? C.jade : C.gold} />
<Row label=“Score match” value={offer.score || 0} />
<Row label=“Vision” value={offer.vision_verdict || “—”} />
{(offer.c24_close > 0) && <>
<Row label=“C24 cierre” value={`€${Math.round(offer.c24_close)}`} />
<Row label=“C24 ROI” value={`${Math.round((offer.c24_roi || 0) * 100)}%`} />
</>}
</div>
{offer.decision_reason && (
<div className=“pt-2” style={{ borderTop: `1px solid ${C.line}` }}>
<div className=“text-[10px] tracking-widest uppercase” style={{ color: C.mute }}>Decisión bot</div>
<div className=“text-xs mt-1” style={{ color: C.dim }}>{offer.decision_bot} · {offer.decision_reason}</div>
</div>
)}
{offer.url && (
<a href={offer.url} target=”_blank” rel=“noopener noreferrer”
className=“block py-2.5 px-4 rounded-lg text-center text-xs font-semibold mt-3”
...
.filter(w => w.status === “sold” || w.status === “returned” || w.status === “lost”)
.sort((a, b) => {
const dateA = a.sale_date || a.returned_date || a.lost_date || a.purchase_date || “”;
const dateB = b.sale_date || b.returned_date || b.lost_date || b.purchase_date || “”;
...
if (period === "today") {
  ...
  prevStart = ymdLocal(firstOfPrevMonth);
  prevEnd = ymdLocal(cutoffPrev);
  const monthsShort = ["ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"];
  ...
  prevStart = ymdLocal(firstOfPrevYear);
  prevEnd = ymdLocal(cutoffPrevY);
  prevLabel = `${y - 1}`;
  daysElapsed = elapsed;
} else { // last30
  const start30 = new Date(t); start30.setDate(start30.getDate() - 29);
  start = ymdLocal(start30);
  end = todayStr;
  label = "Últimos 30 días";
  const start60 = new Date(t); start60.setDate(start60.getDate() - 59);
  const end60 = new Date(t); end60.setDate(end60.getDate() - 30);
...
return { count: filtered.length, revenue, cost, profit };
}, [filtered, state.settings]);

const tabs = [
{ id: “sold”, label: “Vendidos”, count: allSales.filter(w => w.status === “sold”).length, color: C.jade },
{ id: “returned”, label: “Devueltos”, count: allSales.filter(w => w.status === “returned”).length, color: C.amber },
{ id: “lost”, label: “Perdidos”, count: allSales.filter(w => w.status === “lost”).length, color: C.ruby },
{ id: “all”, label: “Todo”, count: allSales.length, color: C.cream },
];
...
const groups = {};
filtered.forEach(w => {
const date = w.sale_date || w.returned_date || w.lost_date || w.purchase_date || “”;
...
const statusChip = (w) => {
if (w.status === “sold”) return <Chip color={C.jade}>Vendido</Chip>;
if (w.status === “returned”) return <Chip color={C.amber}>Devuelto</Chip>;
if (w.status === “lost”) return <Chip color={C.ruby}>Perdido</Chip>;
};
...
<div className="px-4 pb-24 pt-4 space-y-3">
<div className="pt-2">
<h1 className=“font-serif text-3xl” style={{ color: C.cream, fontFamily: “‘Fraunces’, serif”, fontWeight: 300 }}>Ventas</h1>
<div className=“text-xs mt-1” style={{ color: C.dim }}>
{allSales.filter(w => w.status === “sold”).length} exitosas · {allSales.filter(w => w.status === “returned”).length} devueltas · {allSales.filter(w => w.status === “lost”).length} perdidas
</div>
</div>
...
```
  ...
  {grouped.length === 0 ? (
    <Card className="p-8 text-center">
      <ShoppingBag size={32} className="mx-auto mb-3" style={{ color: C.mute }} />
      <div className="text-sm" style={{ color: C.dim }}>Sin resultados</div>
    </Card>
  ) : (
    grouped.map(([month, items]) => {
      const monthRevenue = items.filter(w => w.status === "sold").reduce((s, w) => s + (w.sale_price || 0), 0);
      return (
        <div key={month} className="space-y-2">
          <div className="flex items-baseline justify-between pt-2 px-1">
            <div className="text-xs tracking-widest uppercase font-bold" style={{ color: C.gold }}>
              {monthLabel(month)}
            </div>
            <div className="text-xs" style={{ color: C.mute }}>
              {items.length} op{items.length === 1 ? "" : "s"} · {euro(monthRevenue)}
            </div>
          </div>
          {items.map(w => {
            const d = saleDate(w);
            const dayStr = d ? d.slice(8, 10) : "--";
            const monthStr = d ? ["ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic"][parseInt(d.slice(5, 7)) - 1] : "";
            return (
                ...
                <Card className="p-3.5">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline justify-between gap-2">
                        <span className="font-serif text-base truncate" style={{ color: C.cream, fontFamily: "'Fraunces', serif" }}>{w.brand}</span>
                        <span className="text-sm font-semibold flex-shrink-0" style={{ color: C.gold }}>
                          {w.sale_price ? euroExact(w.sale_price) : "—"}
                        </span>
                      </div>
                      <div className="text-xs truncate" style={{ color: C.dim }}>{w.model}</div>
                      <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                        {statusChip(w)}
                        {w.sale_channel && <Chip color={C.mute}>{w.sale_channel}</Chip>}
                        {w.customer_country && <Chip color={C.copper}>{w.customer_country}</Chip>}
                      </div>
                    </div>
                  </div>
                </Card>
...
// VIEW: SETTINGS / DATA
// ═══════════════════════════════════════════════════════════
const SettingsView = ({ state, dispatch, syncStatus, syncPush, syncPull, setView }) => {
...
return (
<div className=“timelab-bottom-nav fixed bottom-0 left-0 right-0 px-2 pb-2 pt-2” style={{
...
case “add-watch”: return <WatchForm state={state} dispatch={dispatch} prefill={view.prefill} setView={setView} goBack={goBack} />;
...
case “opportunities”: return <OffersInboxView state={state} dispatch={dispatch} setView={setView} />;
case “opportunities-manual”: return <OpportunitiesView state={state} dispatch={dispatch} setView={setView} />;
...
```
    ...
    /* Each view uses "pt-4 pb-24" — we extend those values with env() */
    .max-w-md > div[class*="pt-4"] {
      padding-top: calc(1rem + env(safe-area-inset-top, 0px)) !important;
    }
    .max-w-md > div[class*="pb-24"] {
